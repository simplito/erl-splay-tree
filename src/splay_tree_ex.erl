...
%%
%% ```
%% Tree = splay_tree_ex:new().
%% true = splay_tree_ex:is_empty(Tree).
%% '''
-spec new() -> tree().
new() -> nil.
...
%% 0 = splay_tree_ex:size(Tree0).
%%
%% Tree1 = splay_tree_ex:store(foo, bar, Tree0).
%% 1 = splay_tree_ex:size(Tree1).
%% '''
...
%%
%% ```
%% Tree0 = splay_tree_ex:from_list([{a,1},{b,2},{c,3}]).
%% {3, Tree1} = splay_tree_ex:update_size(Tree0).
%% {3, Tree1} = splay_tree_ex:update_size(Tree1).
%% '''
-spec update_size(tree()) -> {size(), tree()}.
update_size(nil) -> {0, nil};
update_size({_, _, _, _, Size, _} = Node) when Size >= 0 -> {Size, Node};
update_size({K, V, Lft, Rgt, -1, Val}) ->
...
%% in consecutive call to reduce/3 function.
%%
%% == Example ==
%%
%% ```
...
%% Tree0 = splay_tree_ex:new().
%% Tree1 = splay_tree_ex:store(foo, bar, Tree0).
%% Tree2 = splay_tree_ex:store(111, 222, Tree1).
%%
%% [{111, 222}, {foo, bar}] = splay_tree_ex:to_list(Tree2).
%% '''
...
%%
%% ```
%% Tree0 = splay_tree_ex:from_list([{foo, bar}]).
%%
%% %% `foo' exists.
%% Tree1 = splay_tree_ex:update(foo, fun (bar) -> baz end, qux, Tree0).
%% {{ok, baz}, _} = splay_tree_ex:find(foo, Tree1).
%%
%% %% `111' does not exist.
%% Tree2 = splay_tree_ex:update(111, fun (_) -> 222 end, 333, Tree1).
%% {{ok, 333}, _} = splay_tree_ex:find(111, Tree2).
%% '''
...
%%
%% %% `foo' exists.
%% {ok, Tree1} = splay_tree_ex:update(foo, fun (bar) -> baz end, Tree0).
%% {{ok, baz}, _} = splay_tree_ex:find(foo, Tree1).
%%
%% %% `111' does not exist.
%% error = splay_tree_ex:update(111, fun (_) -> 222 end, Tree1).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{foo, bar}]).
%%
%% {{ok, bar}, _} = splay_tree_ex:find(foo, Tree).
%% {error, _} = splay_tree_ex:find(baz, Tree).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{333, 444}, {111, 222}]).
%% {{ok, 333, 444}, _} = splay_tree_ex:find_largest(Tree).
%% '''
...
%% ```
%% Tree = splay_tree_ex:from_list([{333, 444}, {111, 222}]).
%% {{ok, 111, 222}, _} = splay_tree_ex:find_smallest(Tree).
%% '''
...
%%
%% ```
%% Tree0 = splay_tree_ex:from_list([{333, 444}, {111, 222}]).
%% {{ok, 333, 444}, Tree1} = splay_tree_ex:take_largest(Tree0).
%% {{ok, 111, 222}, Tree2} = splay_tree_ex:take_largest(Tree1).
%% {error,          Tree2} = splay_tree_ex:take_largest(Tree2).
%% '''
...
%% ```
%% Tree0 = splay_tree_ex:from_list([{333, 444}, {111, 222}]).
%% {{ok, 111, 222}, Tree1} = splay_tree_ex:take_smallest(Tree0).
%% {{ok, 333, 444}, Tree2} = splay_tree_ex:take_smallest(Tree1).
%% {error,          Tree2} = splay_tree_ex:take_smallest(Tree2).
%% '''
...
%% Tree = splay_tree_ex:from_list([{foo, bar}]).
%%
%% {ok, bar} = splay_tree_ex:lookup(foo, Tree).
%% error = splay_tree_ex:lookup(baz, Tree).
%% '''
...
%% Tree = splay_tree_ex:from_list([{foo, bar}]).
%%
%% bar = splay_tree_ex:get_value(foo, Tree, qux).
%% qux = splay_tree_ex:get_value(baz, Tree, qux).
%% '''
...
%% Tree0 = splay_tree_ex:from_list([{foo, bar}]).
%%
%% Tree1 = splay_tree_ex:erase(foo, Tree0).
%% error = splay_tree_ex:lookup(foo, Tree1).
%%
%% Tree1 = splay_tree_ex:erase(foo, Tree1).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{1, a}, {2, b}, {3, c}]).
%% {Left, Right} = splay_tree_ex:split(2, Tree).
%%
%% [1] = splay_tree_ex:keys(Left).
%% [2, 3] = splay_tree_ex:keys(Right).
%% '''
...
%% ```
%% Tree = splay_tree_ex:from_list([{1, a}, {2, b}, {3, c}]).
%% {{ok, 2, b}, _} = splay_tree_ex:find_lower_bound(2, Tree).
%% {{ok, 3, c}, _} = splay_tree_ex:find_lower_bound(2.5, Tree).
%% {error, _}      = splay_tree_ex:find_lower_bound(3.1, Tree).
%% '''
...
find_lower_bound(Key, Tree) ->
    ...
    case Right of
        nil            -> {error, Left};
        {K, V, nil, _, _, _} -> {{ok, K, V}, lft(Right, Left)}
    end.
...
%% ```
%% Tree = splay_tree_ex:from_list([{1, a}, {2, b}, {3, c}]).
%% {{ok, 3, c}, _} = splay_tree_ex:find_upper_bound(2, Tree).
%% {{ok, 3, c}, _} = splay_tree_ex:find_upper_bound(2.5, Tree).
%% {error, _}      = splay_tree_ex:find_upper_bound(3.1, Tree).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{2, b}, {3, c}, {1, a}]).
%% [{1, a}, {2, b}, {3, c}] = splay_tree_ex:to_list(Tree).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{2, a}, {3, b}, {1, c}]).
%% [c, a, b] = splay_tree_ex:values(Tree).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{2, b}, {1, a}]).
%% {{ok, a}, _} = splay_tree_ex:find(1, Tree).
%% '''
...
%%
%% ```
%% Tree0 = splay_tree_ex:from_list([{1, 2}, {3, 4}]).
%% Tree1 = splay_tree_ex:map(fun (K, V) -> K + V end, Tree0).
%% [{1, 3}, {3, 7}] = splay_tree_ex:to_list(Tree1).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{a, 1}, {b, 2}]).
%% [2, 1] = splay_tree_ex:foldl(fun (_, V, Acc) -> [V | Acc] end, [], Tree).
%% '''
...
%% ```
%% Tree = splay_tree_ex:from_list([{a, 1}, {b, 2}]).
%% [1, 2] = splay_tree_ex:foldr(fun (_, V, Acc) -> [V | Acc] end, [], Tree).
%% '''
...
%% ```
%% Tree = splay_tree_ex:from_list([{a, 1}, {b, 2}]).
%% [1] = splay_tree_ex:foldl_while(fun (_, V, Acc) -> {false, [V | Acc]} end, [], Tree).
%% '''
...
%% ```
%% Tree = splay_tree_ex:from_list([{a, 1}, {b, 2}]).
%% [2] = splay_tree_ex:foldr_while(fun (_, V, Acc) -> {false, [V | Acc]} end, [], Tree).
%% '''
...
%%
%% ```
%% Tree0 = splay_tree_ex:from_list([{aaa, bbb}, {111, 222}]).
%% Tree1 = splay_tree_ex:filter(fun (K, _) -> is_atom(K) end, Tree0).
%% [{aaa, bbb}] = splay_tree_ex:to_list(Tree1).
%% '''
...
%%
%% ```
%% Tree = splay_tree_ex:from_list([{a, p}, {b, q}, {c, r}, {d, s}, {e, t} ]).
%%
%% {{ok, 4}, _} = splay_tree_ex:index(d, Tree).
%% {error, _} = splay_tree_ex:index(baz, Tree).
%% '''
...
%% Tree = splay_tree_ex:from_list([{a, p}, {b, q}, {c, r}, {d, s}, {e, t} ]).
%%
%% {{ok, c, r}, _} = splay_tree_ex:at(3, Tree).
%% {error, _} = splay_tree_ex:at(baz, Tree).
%% '''
...
foldl_node(Fun, {Key, Val, Lft, Rgt, _, _}, Acc) -> foldl_node(Fun, Rgt, Fun(Key, Val, foldl_node(Fun, Lft, Acc))).

-spec foldr_node(fold_fn(), maybe_tree_node(), term()) -> term().
foldr_node(_Fun, nil, Acc)                 -> Acc;
foldr_node(Fun, {Key, Val, Lft, Rgt, _, _}, Acc) -> foldr_node(Fun, Lft, Fun(Key, Val, foldr_node(Fun, Rgt, Acc))).
...
foldl_while_node(Fun, {Key, Val, Lft, Rgt, _, _}, Acc) -> foldl_while_node(Fun, Rgt, ?MAYBE_BREAK(Fun(Key, Val, foldl_while_node(Fun, Lft, Acc)))).

-spec foldr_while_node(fold_while_fn(), maybe_tree_node(), term()) -> term().
foldr_while_node(_Fun, nil, Acc)                 -> Acc;
foldr_while_node(Fun, {Key, Val, Lft, Rgt, _, _}, Acc) -> foldr_while_node(Fun, Lft, ?MAYBE_BREAK(Fun(Key, Val, foldr_while_node(Fun, Rgt, Acc)))).
